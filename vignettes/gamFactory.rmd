...
head(chicago)
```
The data set contains daily measurements of air pollution and mortality in Chicago. Before modelling the data with `gamFactory`, we pre-process the data so that it has no missing values and follows the format requirements needed by the nested effects which will be used below. The modified data is obtained via `chicago_data` function given below:
